...
- **Target baton version for parity:** `6.1.0` _(bumped from `6.0.0` 2026-07-23 — upstream released 6.0.1 and 6.1.0; see Changelog for this file, below, for what the re-pin required.)_
...
- **Short-form JSON aliases:** types accept both long and short forms via serde `alias`; emit long form on serialise. Originally adopted for `Avu` (`attribute`/`value`/`units` ↔ `a`/`v`/`u`, Session 1); extended to baton-do envelope keys (`operation`/`arguments` ↔ `op`/`args`, Session 7) to mirror upstream baton's CLI shorthand. Any new wire type with multiple accepted spellings follows the same pattern: long form is canonical on emit, short form is accepted on parse.
...
- **Envelope round-trip fidelity:** `BatonDoEnvelope` and `BatonDoOutput` carry an `extra: BTreeMap<String, serde_json::Value>` field that captures top-level keys baton-rs doesn't model. The dispatcher copies `envelope.extra` → `output.extra`, and `#[serde(flatten)]` puts those entries back at the top level on serialise — so any consumer-supplied field round-trips verbatim. This matches upstream baton's pattern (`baton/src/operations.c:130-150`: parse input JSON, attach `result`/`error` to the parsed object, `print_json(item)` the modified original — *not* a new object built from typed extractions). Load-bearing for extendo's `ID` request-response correlation field (`extendo/client.go:97-100, 382-396`). Adding new typed fields to the envelope is fine; *removing* the round-trip is not — every untyped consumer field would silently break downstream. Adopted post-Session-8 in the wake of the extendo CI work.
...
- `2026-05-12` — Extendo compatibility workflow shipped on `feat/extendo-ci` (#84, closes #76); 97/97 Ginkgo specs passing in ~71 s informational on every PR + push to main. Four wire-compat fixes surfaced and landed on `main` in the process:
  - **#80** — envelope ID round-trip via `BatonDoEnvelope.extra` flatten map. Extendo correlates responses by a top-level `"ID"` field; baton-rs was stripping unknown envelope keys, response readers hung. Now matches upstream baton's "modify input JSON in place" pattern (`baton/src/operations.c:130-150`). New cross-cutting convention: "Envelope round-trip fidelity" (Cross-cutting conventions above).
  - **#81** — put honours `irods_default_resource` from the client env. Shim adds `DEF_RESC_NAME_KW` to the `rcDataObjOpen` condInput on write, and the post-put checksum read switches to `CHKSUM_ALL_KW | FORCE_CHKSUM_KW` so the catalog is populated on multi-replica resources before we read it back.
...
- `2026-07-31` — Connection recovery for mid-stream drops (#108) on `feat/reconnect-on-connection-error`. Previously one connection-level error poisoned every subsequent NDJSON record — the dead connection was reused until the `--connect-time` watchdog fired. Added `error::is_connection_level_code` (iRODS socket/transport bases from `rodsErrorTable.h` with the errno sub-code stripped; includes the generic `SYS_INTERNAL_ERR` (-154000) the client surfaces for a dropped socket) and `ReconnectingSession::note_error`, which flags a rebuild that the next `maybe_reconnect` performs before any op runs — so no op touches a dead handle. The in-flight record is annotated in-band and **not** retried (a half-applied write is never re-issued); reconnect failure stays fatal, as the age-based path already is. Wired into the six annotate-and-continue loops (list/get/put/chmod/metamod/baton-do); `baton-metaquery`'s main query is already fail-fast (`?`), so it has no cross-record cascade and was left as-is. New test-only `fault-injection` Cargo feature gates a `shim_test_force_disconnect` seam (`shutdown(SHUT_RDWR)`) backing an integration test that asserts recovery across the iRODS 4.2.7/4.3.4/4.3.5 matrix. Version bump + CHANGELOG entry deferred to a future `release: 1.1.0` per the release-commit convention.
