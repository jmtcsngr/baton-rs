...
- `2026-07-23` — Target baton version for parity bumped `6.0.0` → `6.1.0` on branch `chore/bump-baton-compat-6.1.0` (PR #102), landed as a compat *feature*, not a version-string-only chore. Upstream had shipped two releases we hadn't tracked: `6.0.1` (2026-05-19, mostly CI/dependency churn plus two OOB-write hardening fixes in its genQuery builder, wtsi-npg/baton#337/#338) and `6.1.0` (2026-07-02, adds a `physical_path` field to replicate JSON objects, wtsi-npg/baton#339). Two gaps were filed and then actually closed before `BATON_COMPAT_VERSION` moved:
  - **#100** — baton-rs's own genQuery builder (`shim_query_add_where` / `shim_query_add_select`) had the identical unbounded-condition-count gap upstream just patched: `addInxVal` / `addInxIval` write into iRODS's fixed-size `sqlCondInp` / `selectInp` arrays with no bounds check, and `operations::metaquery`'s `timestamps` / `access` arrays are unbounded caller-supplied JSON. Fixed by checking `len >= MAX_NUM_CONDITIONS` in the shim before each append and returning `CAT_INVALID_ARGUMENT` (auto-resolved by the existing `BatonError::from_irods` → `rodsErrorName` path, no new lookup-table entry needed). `GenQuery::add_select` had to grow a `Result` return (previously infallible) — 25 call sites across `list.rs` / `metaquery.rs` updated. New integration test in `tests/metaquery.rs` drives 2000 `access` entries and asserts a clean in-band error rather than a crash.
