...
// Test-only seam (issue #108): compiled only under
// `-DBATON_RS_FAULT_INJECTION`, set by build.rs for
// `--features fault-injection`. Never in a release build.
#ifdef BATON_RS_FAULT_INJECTION
// Shut down the connection's TCP socket so the next operation fails as
// if the server dropped it, without freeing the handle. Returns 0, or
// -1 on a NULL handle.
...
// Generous: real `relVersion` strings on iRODS 4.x are well under 32
// bytes (e.g. "rods4.3.5"), but iRODS itself sizes the field at
// NAME_LEN (64) and the actual buffer the server sends could grow.
#define SHIM_SERVER_VERSION_LEN 128

// Fetch the server's `relVersion` string via `rcGetMiscSvrInfo`,
// copy it NUL-terminated into `out`, and free the underlying iRODS
// struct. Returns 0 on success or the iRODS error code on failure.
//
// Returns the raw `relVersion` (e.g. "rods4.3.5") rather than the
// regex-extracted X.Y.Z — Rust callers handle the trailing-version
// parse so the shim stays minimal. Used by `baton-do --server-version`
// to mirror upstream's health-check probe.
...
// Sized for SHA256 (64 hex chars) + an algorithm prefix + headroom.
// `baton-put`'s --checksum and --verify paths need the server-side
// digest in a caller-supplied buffer.
#define SHIM_CHECKSUM_LEN 256

// Bit flags for `shim_data_obj_checksum`'s `flags` parameter. Mirror
// upstream baton's CALCULATE / VERIFY / FORCE flag triple
// (baton/src/read.h, applied at baton/src/read.c:610-625) so the
// keyword choice on `rcDataObjChksum` matches upstream wire semantics.
//
// CALCULATE adds CHKSUM_ALL_KW (compute on all replicates).
// VERIFY    adds VERIFY_CHKSUM_KW (re-verify recorded digest).
// FORCE     adds FORCE_CHKSUM_KW (only meaningful with CALCULATE).
//
// CALCULATE and VERIFY are mutually exclusive; passing both returns
// USER_INPUT_OPTION_ERR. flags=0 issues `rcDataObjChksum` with no
// keywords (the post-put-checksum behaviour kept for the put.rs
// caller in `baton-put` --checksum / --verify).
#define SHIM_CHKSUM_CALCULATE 0x1
#define SHIM_CHKSUM_VERIFY    0x2
#define SHIM_CHKSUM_FORCE     0x4

// Compute or retrieve the iRODS-side checksum for `path` and copy
// the resulting NUL-terminated string into `out`. Wraps
// `rcDataObjChksum`, which computes a fresh digest when none is
// recorded and returns the stored value otherwise — sufficient for
// post-put callers, since a freshly-written object does not yet have
// a server-side digest unless server policy auto-computed one on
// close.
//
// `flags` is a bitfield of `SHIM_CHKSUM_*` constants. flags=0 gives
// the keyword-less behaviour used by the post-put caller. The
// dispatcher path uses CALCULATE / VERIFY / FORCE per upstream.
//
// Returns 0 on success. On error returns the iRODS error code and
// sets `out[0] = '\0'` so the caller never reads a stale digest from
// a previous call.
//
// The format of the returned string depends on the iRODS-side hash
// scheme. Plain MD5 (the client default pinned for issue #25) yields
// 32 hex characters. SHA2 yields a `sha2:` prefix followed by base64.
// `baton-put` strips the prefix before comparing against its
// client-side hex digest.
...
// ---- Path management --------------------------------------------------------
//
// Wrappers for the iRODS namespace operations that `baton-do`
// dispatches to: rename (move), unlink (data-object remove), rmcoll
// (collection remove), and collection-create (mkdir). Each one is
// a thin shell over the corresponding iRODS C call; the
// `condInput` `keyValPair_t` plumbing for force / recurse keywords
// stays inside the shim so the Rust side never sees iRODS's
// kvp / kw conventions.

// Rename (move) `src_path` to `dest_path`. iRODS uses a single
// `rcDataObjRename` API for both kinds of namespace entry but
// needs the right `oprType` opcode — `source_is_collection` (0/1)
// switches between `RENAME_DATA_OBJ` and `RENAME_COLL`. The Rust
// caller already knows the source type from its `Target` enum, so
// passing it explicitly avoids a redundant server-side stat.
//
// Cross-collection moves work — iRODS resolves `dest_path`
// against its catalog with no client-side scoping check (matches
// upstream baton).
int shim_rename(
    shim_rods_conn_t *conn,
    const char       *src_path,
...
// Remove a data object. `FORCE_FLAG_KW` is set unconditionally,
// matching upstream baton's "no soft delete" semantics — the
...
// Remove a collection. `recurse=1` sets `RECURSIVE_OPR__KW` so
// non-empty collections are removed in one call; without it
// iRODS returns `CAT_COLLECTION_NOT_EMPTY` against a populated
// target. `force=1` sets `FORCE_FLAG_KW` for permanent deletion
...
// Create a collection at `path`. `recurse=1` sets
// `RECURSIVE_OPR__KW` so intermediate parents are created (mkdir
// -p semantics); without it iRODS errors when the parent
// doesn't exist.
