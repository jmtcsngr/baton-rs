...
#ifdef BATON_RS_FAULT_INJECTION
// Test-only fault injection (issue #108). See ffi_shim.h for the
// feature gating.
#include <sys/socket.h>

// shutdown(SHUT_RDWR), not close(): it breaks the TCP connection so the
// next rcXxx call fails, but keeps the fd valid (no fd-reuse race) and
// leaves rcComm_t allocated so the reconnect-and-retry path under test
// can still use the handle. rcComm_t exposes the socket as `int sock`.
int shim_test_force_disconnect(shim_rods_conn_t *conn) {
    if (!conn) return -1;
    shutdown(((rcComm_t *)conn)->sock, SHUT_RDWR);
    return 0;
}
#endif
...
int shim_get_server_version(
    shim_rods_conn_t *conn,
    char             *out,
    int               out_len)
{
    if (!conn || !out || out_len <= 0) return -1;
    out[0] = '\0';

    miscSvrInfo_t *info = NULL;
    int status = rcGetMiscSvrInfo((rcComm_t *)conn, &info);
    if (status < 0) {
        if (info) free(info);
        return status;
    }
    if (!info) {
        // Defensive: a non-error return without an info struct would
        // be an iRODS protocol violation. Surface as a generic error.
        return -1;
    }

    strncpy(out, info->relVersion, (size_t)(out_len - 1));
    out[out_len - 1] = '\0';
    // Upstream baton 6.0.0 leaks this struct (`baton/src/baton.c:232`
    // never frees it); we free it for hygiene since iRODS allocates
    // it via malloc inside rcGetMiscSvrInfo.
    free(info);
    return 0;
}
    ...
    shim_open_mode_t  mode)
{
    ...
    case SHIM_OPEN_WRITE:
        ...
        // Pass the client-env default resource as DEF_RESC_NAME_KW
        // so a put without an explicit destination resource lands
        // on the resource the user configured in
        // `irods_environment.json` rather than the server-side
        // fallback (typically `testResc` on the wtsi-npg test
        // images, regardless of what the client env says). Mirrors
        // upstream baton's `write.c:128-130` (the env value comes
        // from `getRodsEnv` at `operations.c:672-675`). Without
        // this keyword, the iRODS client lib ignores
        // `irods_default_resource` for `rcDataObjOpen(O_CREAT)` —
        // surfaced by extendo's replicate test, which expects two
        // replicas on `replResc`'s children and was getting one
        // on `testResc` instead.
        {
            rodsEnv env;
            memset(&env, 0, sizeof(env));
            if (getRodsEnv(&env) == 0 &&
                strnlen(env.rodsDefResource, NAME_LEN) > 0) {
                addKeyVal(&inp.condInput, DEF_RESC_NAME_KW,
                          env.rodsDefResource);
                has_kw = 1;
            }
        }
    ...
    const int status = rcDataObjOpen((rcComm_t *)conn, &inp);
    if (has_kw) {
    ...
    int               out_len)
{
    ...
    if (!chksum) {
        // VERIFY mode legitimately returns success with chksum=NULL —
        // `rcDataObjChksum` only populates *chksum when computing
        // (CALCULATE) or reading the catalog (no keywords). For
        // VERIFY the call is "did the data match"; an empty
        // out-buffer plus 0 status is the right shape. See #57
        // (Cluster E) and partisan's `test_verify_checksum_good`,
        // which probes this with a known-good object. The Rust
        // wrapper handles the empty buffer by fetching the catalog
        // digest separately so callers always see a string on a
        // successful verify.
        //
        // Non-VERIFY success with chksum=NULL would still be an
        // iRODS protocol violation; flag it as -1 so the caller has
