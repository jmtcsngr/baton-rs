...
fn dispatch_inner(
    ...
    match envelope.operation {
        ...
        Operation::Checksum => {
            ...
            let opts = checksum_options_from_arguments(&envelope.arguments);
            let out = checksum::checksum_one_annotated(conn, target, &opts);
            split_result(out)
        }
        Operation::Mv => {
            let target = match expect_standard(&envelope.target) {
...
/// Map baton-do envelope arguments onto
/// [`checksum::ChecksumOptions`] for the standalone `checksum`
/// op. Default-when-absent matches upstream baton
/// (`baton/src/read.c:598-602`): when neither `checksum` nor
/// `verify` is set, fall back to CALCULATE so the wire shape
/// `{op:"checksum", target:...}` with empty arguments still
/// triggers a server-side calculation. Mutual exclusion of
/// `checksum` + `verify` is rejected at `checksum_one`, mirroring
/// upstream's USER_INPUT_OPTION_ERR (`baton/src/read.c:603-607`).
fn checksum_options_from_arguments(args: &Arguments) -> checksum::ChecksumOptions {
    let calculate = args.checksum;
    let verify = args.verify;
    let force = args.force;
    if !calculate && !verify {
        // Upstream's default. Without this, `rcDataObjChksum`
        // would receive no keywords and behave differently from
        // upstream baton's `baton-do checksum` with empty args.
        return checksum::ChecksumOptions {
            calculate: true,
            verify: false,
            force,
        };
...
fn metamod_input_from_envelope(
    ...
    let op_str = arguments.operation.as_deref().ok_or_else(|| {
        // -403000 USER_INPUT_OPTION_ERR — missing required arg.
        BatonError::from_irods_with_context(
            -403000,
            "metamod: arguments.operation is required (\"add\" or \"rem\")",
        )
    })?;
    let operation = match op_str {
        ...
        other => {
            // -403000 USER_INPUT_OPTION_ERR — unknown enum
            // discriminator value on a required arg.
...
mod tests {
    ...
    // --- checksum_options_from_arguments ------------------------------------

    #[test]
    fn checksum_options_empty_arguments_default_to_calculate() {
        // Wire-faithful default: empty arguments must trigger
        // CALCULATE so the keyword on rcDataObjChksum matches
        // upstream baton's `baton-do checksum {}` behaviour
        // (`baton/src/read.c:598-602`).
        let opts = checksum_options_from_arguments(&empty_arguments());
        assert!(opts.calculate);
        assert!(!opts.verify);
        assert!(!opts.force);
    }

    #[test]
    fn checksum_options_checksum_arg_sets_calculate() {
        let mut args = empty_arguments();
        args.checksum = true;
        let opts = checksum_options_from_arguments(&args);
        assert!(opts.calculate);
        assert!(!opts.verify);
        assert!(!opts.force);
    }
    ...
    #[test]
    fn checksum_options_verify_arg_sets_verify_only() {
        // verify=true must NOT set calculate — they're mutually
        // exclusive at the layer below. The default fallback only
        // applies when neither flag is set.
        let mut args = empty_arguments();
        args.verify = true;
        let opts = checksum_options_from_arguments(&args);
        assert!(!opts.calculate);
    ...
    #[test]
    fn checksum_options_force_threads_through() {
        let mut args = empty_arguments();
        args.checksum = true;
        args.force = true;
        let opts = checksum_options_from_arguments(&args);
    ...
    #[test]
    fn checksum_options_verify_and_checksum_passes_both_through_for_layer_below() {
        // The dispatcher does NOT silently resolve the conflict —
        // it lets `checksum_one` reject the combination (mirroring
        // upstream's USER_INPUT_OPTION_ERR). Pin that both flags
        // arrive at the operations layer untouched.
        let mut args = empty_arguments();
        args.checksum = true;
        args.verify = true;
        let opts = checksum_options_from_arguments(&args);
