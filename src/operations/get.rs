...
//! the output JSON's `data` field. `--save` file mode streams bytes
//! directly to disk and skips the inline `data` field. The local
//! path follows upstream baton's `json_to_local_path` priority
...
pub struct GetOptions {
    ...
    /// When `false`, the inline base64 path is used.
    pub save: bool,
