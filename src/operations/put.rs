...
pub fn put_one(
    ...
    if opts.checksum || opts.verify {
        // No keywords on this call — keeps the post-put behaviour
        // unchanged (just read whatever digest the catalog has,
        // computing if missing). The dispatcher path on the
        // standalone `checksum` op uses CALCULATE/VERIFY/FORCE
