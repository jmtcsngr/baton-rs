...
pub fn list_one(
    ...
    // Mirror upstream baton's implicit target-type resolution: if the
    // caller's input shape doesn't match what iRODS reports, re-shape
    // the target before listing. Drives partisan's
    // `partisan.irods.rods_path_type` (`partisan/src/partisan/irods.py:1663`),
    // which sends `{"collection": full_path}` regardless of the path's
    // catalog identity and pattern-matches on whether the response also
    // carries a `data_object` field. See #57 (Issue E).
    target = resolve_target_type(target, &stat);
    ...
    if let Target::DataObject(d) = &mut target {
        if opts.size || opts.checksum {
            ...
            // --size or --checksum. Possible future optimisation:
            // skip the stat when we're going to query the catalog
            // anyway. Not on the critical path; no open issue.
            ...
            if opts.checksum {
                // Always emit the field when `--checksum` is requested,
                // even if the catalog hasn't recorded one yet (empty
                // string for a freshly-put data object on a server
                // without auto-checksum policy). Matches upstream
                // baton's wire shape — partisan's
                // `DataObject.checksum()` (`irods.py:2503`)
                // unconditionally accesses `item[Baton.CHECKSUM]`
                // after a `_list(checksum=True)` and would otherwise
                // raise `KeyError: 'checksum'`. See #57 (Cluster B).
...
mod tests {
    ...
    // --- split_path / resolve_target_type --------------------------------

    #[test]
    fn split_path_handles_typical_data_object_path() {
        let (parent, basename) = split_path("/zone/home/u/file.txt");
        assert_eq!(parent, "/zone/home/u");
        assert_eq!(basename, "file.txt");
    }

    #[test]
    fn split_path_root_collection_keeps_leading_slash() {
        // Defensive: a path like "/foo" splits at the leading `/`.
        // The parent should be "/" (the root), not the empty string,
        // so callers can join it back to the basename without losing
        // the absolute-path leading slash.
        let (parent, basename) = split_path("/foo");
        assert_eq!(parent, "/");
        assert_eq!(basename, "foo");
    }

    #[test]
    fn split_path_no_slash_returns_empty_parent() {
        // Defensive against degenerate input. iRODS paths are always
        // absolute, so this branch shouldn't fire in practice.
        let (parent, basename) = split_path("foo");
        assert_eq!(parent, "");
        assert_eq!(basename, "foo");
    }
    ...
    #[test]
    fn resolve_target_type_collection_to_data_object_splits_path() {
        let target = Target::Collection(Collection {
            collection: "/zone/home/u/file.txt".to_string(),
            avus: None,
            access: None,
            timestamps: None,
            contents: None,
        ...
        let stat = ObjStat {
            size: 12,
            checksum: String::new(),
            obj_type: ObjType::DataObject,
        };
        let resolved = resolve_target_type(target, &stat);
        match resolved {
            Target::DataObject(d) => {
                assert_eq!(d.collection, "/zone/home/u");
                assert_eq!(d.data_object, "file.txt");
            }
            other => panic!("expected DataObject after resolve, got {:?}", other),
        }
    }

    #[test]
    fn resolve_target_type_data_object_to_collection_joins_path() {
        let target = Target::DataObject(DataObject {
            collection: "/zone/home/u".to_string(),
            data_object: "thing".to_string(),
            size: None,
            checksum: None,
            data: None,
            directory: None,
        ...
        let resolved = resolve_target_type(target, &stat);
        match resolved {
            Target::Collection(c) => assert_eq!(c.collection, "/zone/home/u/thing"),
            other => panic!("expected Collection after resolve, got {:?}", other),
        }
    }

    #[test]
    fn resolve_target_type_passes_through_when_already_correct() {
        let target = Target::Collection(Collection {
            collection: "/zone/home/u".to_string(),
            avus: None,
            access: None,
    ...
    #[test]
    fn resolve_target_type_passes_through_for_other_obj_type() {
        // ObjType::Other (spec-coll links etc.) leaves the input
        // untouched — we don't have a sensible re-shape for non-
        // {data object, collection} types.
        let target = Target::Collection(Collection {
            collection: "/zone/home/u".to_string(),
            avus: None,
            access: None,
...
pub fn list_one_annotated(
    ...
    super::annotate_failure(target, |t| list_one(conn, t, opts))
}

/// Re-shape `target` so its variant matches what `rcObjStat` reports
/// for the path. Mirrors upstream baton's implicit type resolution:
///
/// - `Target::Collection { collection: "/a/b/c.txt" }` + stat says
///   data object → `Target::DataObject { collection: "/a/b",
///   data_object: "c.txt", ... }`. Splits the input path on the
///   last `/`. Other fields reset (caller-provided AVUs / ACL on a
///   misclassified input wouldn't have been meaningful for the
///   resolved type).
/// - `Target::DataObject { collection: "/a/b", data_object: "c" }`
///   + stat says collection → `Target::Collection { collection:
///   "/a/b/c", ... }`. Symmetric for parity.
/// - `ObjType::Other` and same-shape cases: pass through unchanged.
fn resolve_target_type(target: Target, stat: &ObjStat) -> Target {
    match (&target, &stat.obj_type) {
        (Target::Collection(_), ObjType::DataObject) => {
            let path = target.path();
            let (parent, basename) = split_path(&path);
            Target::DataObject(DataObject {
                collection: parent,
                data_object: basename,
                size: None,
                checksum: None,
                data: None,
                directory: None,
        ...
        (Target::DataObject(_), ObjType::Collection) => {
            let path = target.path();
...
/// Split an iRODS absolute path into (parent collection, basename)
/// at the last `/`. The parent always has a leading `/` (since iRODS
/// paths are absolute) and never has a trailing `/`. A path with no
/// `/` returns (`""`, path) — defensive; iRODS paths always start
/// with `/` so this only fires on degenerate input.
