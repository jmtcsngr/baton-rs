...
impl ReconnectingSession {
    ...
    /// for one-shot setup work before the NDJSON loop begins —
    /// e.g. the `--server-version` health-check probe in
    /// `baton-do` (#55).
...
impl RodsConnection {
    ...
    /// Rename (move) `src_target` to `dest_path`. The source's
    /// path is read from `src_target.path()`; the source-type flag
    /// (`oprType` = `RENAME_DATA_OBJ` vs `RENAME_COLL`) derives
    /// from the [`Target`] variant.
    ///
    /// Wraps `shim_rename`, which calls `rcDataObjRename`
    /// server-side. iRODS performs the catalog update; cross-
    /// collection moves are supported with no client-side scoping
    /// check (matches upstream baton).
    pub fn rename(
        &mut self,
        src_target: &Target,
        dest_path: &str,
    ) -> Result<(), BatonError> {
        let src_c = CString::new(src_target.path()).map_err(|_| BatonError {
            code: -1,
            message: "source path contains interior NUL".to_string(),
        })?;
        let dest_c = CString::new(dest_path).map_err(|_| BatonError {
            code: -1,
            message: "destination path contains interior NUL".to_string(),
        })?;
        let source_is_collection = matches!(src_target, Target::Collection(_));
        ...
        let status = unsafe {
            ffi::shim_rename(
                ...
                src_c.as_ptr(),
                dest_c.as_ptr(),
                if source_is_collection { 1 } else { 0 },
            )
        };
    ...
    /// Remove a data object permanently. `FORCE_FLAG_KW` is set
    /// inside the shim — there is no soft-delete via this method
    /// (matches upstream baton's `remove` operation).
    ///
    /// Path-only because the operations layer validates the
    /// caller's [`Target`] is a data object before reaching this
    /// method. iRODS itself returns `CAT_NO_ACCESS_PERMISSION` or
    /// similar if the path resolves to a collection — the
    /// validation here is just for clearer client-side error
    /// messaging.
    pub fn unlink_data_object(&mut self, path: &str) -> Result<(), BatonError> {
        let path_c = CString::new(path).map_err(|_| BatonError {
            code: -1,
            message: "path contains interior NUL".to_string(),
        })?;

        let status = unsafe { ffi::shim_data_obj_unlink(self.conn, path_c.as_ptr()) };
    ...
    /// Remove a collection. `recurse = true` removes a populated
    /// collection in one call; without it iRODS returns
    /// `CAT_COLLECTION_NOT_EMPTY`. `force = true` removes
    /// permanently rather than trashing — both flags are caller-
    /// controlled to match upstream baton's `rmdir` operation.
    ///
    /// Path-only because the operations layer validates the
    /// caller's [`Target`] is a collection before reaching this
    /// method.
    pub fn rm_collection(
        &mut self,
        path: &str,
        recurse: bool,
        force: bool,
    ...
    /// Create a collection at `path`. `recurse = true` gives
    /// `mkdir -p` semantics — intermediate parents that don't
    /// exist get created in the same call. Without it iRODS
    /// errors when the parent is missing.
    ///
    /// Path-only (rather than [`Target`]-based) because the
    /// collection doesn't exist yet — there's no [`Target`] to
    /// pass.
    ...
    ///
    /// `flags` is a bitfield of `ffi::SHIM_CHKSUM_*` constants
    /// (CALCULATE / VERIFY / FORCE), mirroring upstream baton's flag
    /// triple. flags=0 issues `rcDataObjChksum` with no keywords —
    /// the post-put behaviour used by `baton-put` --checksum / --verify.
    /// Mutually exclusive: CALCULATE + VERIFY together returns
    ...
    pub fn checksum_data_object(
        &mut self,
        path: &str,
        flags: c_int,
    ) -> Result<String, BatonError> {
        let path_c = CString::new(path).map_err(|_| BatonError {
            code: -1,
            message: "path contains interior NUL".to_string(),
        })?;
    ...
    /// returned verbatim (e.g. `"rods4.3.5"`). Used by
    /// `baton-do --server-version` to mirror upstream's health-check
    /// probe; the trailing X.Y.Z extraction lives in the binary
    /// (`extract_server_version` in `src/bin/baton-do.rs`) so the
    /// connection layer stays string-shape-agnostic.
    pub fn server_version_relversion(&mut self) -> Result<String, BatonError> {
        let mut buf: [c_char; ffi::SHIM_SERVER_VERSION_LEN] =
            [0; ffi::SHIM_SERVER_VERSION_LEN];
        let status = unsafe {
            ffi::shim_get_server_version(
                self.conn,
                buf.as_mut_ptr(),
                buf.len() as c_int,
            )
    ...
    /// **Test-only** (`fault-injection` feature, issue #108). Break the
    /// TCP socket to simulate a mid-stream drop; the handle stays valid
    /// so rebuild-and-retry can be exercised.
    #[cfg(feature = "fault-injection")]
    pub fn test_force_disconnect(&mut self) {
        if !self.conn.is_null() {
            unsafe { ffi::shim_test_force_disconnect(self.conn) };
        }
    }
}
