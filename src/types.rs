...
// Last modified by: Claude Sonnet 5 (1M context)
...
pub struct Replicate {
    ...
    /// The replica's physical path on the resource server's
    /// filesystem, sourced from `COL_D_DATA_PATH`. Added in baton
    /// 6.1.0 (wtsi-npg/baton#339); baton-rs always populates it, so
    /// unlike upstream pre-6.1.0 servers there's no "missing key"
    /// case to model here.
...
pub struct DataObject {
    ...
    /// Server-recorded checksum for the data object. Wire shape
    /// has three states distinguished by serde:
    ///
    /// - `None`: field omitted (`--checksum` was not requested,
    ///   or this struct is being used to construct an input
    ///   target).
    /// - `Some("")`: emitted as JSON `null` (`--checksum`
    ///   requested, but the catalog has no recorded checksum —
    ///   e.g. a put without `-K` and no server-side
    ///   auto-checksum policy). Matches upstream baton's wire
    ///   shape and what partisan's
    ///   `DataObject.checksum() is None` assertion expects
    ///   (`partisan/src/partisan/irods.py:2503`).
    /// - `Some(non-empty)`: emitted as JSON string verbatim.
    ///
    /// Distinguishing "omit" from "emit null" requires a custom
    /// `serialize_with` hook because plain `Option<String>` can't
    /// represent three states. The hook only fires when the
    /// outer `Option` is `Some` (i.e. `skip_serializing_if`
    /// already filtered out `None`).
    #[serde(
    ...
    /// Local destination directory for `baton-get --save` /
    /// source directory for `baton-put`. Combined with
    /// [`Self::file`] (preferred) or [`Self::data_object`] to form
    /// the local path. Echoed on output records when present on
    /// input — matches baton's per-record save-destination
    /// convention.
    #[serde(skip_serializing_if = "Option::is_none", default)]
    pub directory: Option<String>,

    /// Local file name, used in preference to [`Self::data_object`]
    /// when constructing the local path on get / put. Mirrors
    /// upstream baton's `JSON_FILE_KEY` (`baton/src/json.h:40`)
    /// and the priority in `json_to_local_path`
    /// (`baton/src/json.c:1211-1244`): `directory` + `file` wins
    /// over `directory` + `data_object`. Lets callers stage bytes
    /// to a local name that differs from the iRODS-side
    /// `data_object` identifier — both extendo
    /// (`rodsitem.go:40`, `IFile string \`json:"file,omitempty"\``)
    /// and partisan (`irods.py:469-470`, `:528-529`) use this
    /// spelling.
    #[serde(skip_serializing_if = "Option::is_none", default)]
...
/// Value side of an [`AvuQuery`]. Most operators (`=`, `like`, `<`,
/// …) take a single string; [`Operator::In`] takes an array of
/// strings. Partisan encodes both shapes on the wire directly —
/// `AVU.value` is passed through `json.dumps` as either a `str` or a
/// `list[str]` (`partisan/src/partisan/irods.py:3908-3915`) — so we
/// accept both via an untagged enum and let the query layer pick
/// the right SQL shape. See partisan's
/// `test_examples.py::test_data_object_examples` for the IN-array
/// use case.
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
#[serde(untagged)]
pub enum AvuValue {
...
impl From<&str> for AvuValue {
    fn from(s: &str) -> Self {
        AvuValue::Single(s.to_string())
    }
}

impl From<String> for AvuValue {
    fn from(s: String) -> Self {
        AvuValue::Single(s)
    }
}

impl From<Vec<String>> for AvuValue {
...
///
/// `value` accepts either a string (most operators) or an array of
/// strings (only meaningful for [`Operator::In`]) — see [`AvuValue`]
/// for the rationale.
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct AvuQuery {
    pub attribute: String,
...
/// Custom serializer for [`DataObject::checksum`]: emits JSON `null`
/// for `Some("")` (catalog has no recorded checksum) and the bare
/// string otherwise. Combined with `skip_serializing_if =
/// "Option::is_none"`, the field has three wire states:
///
/// - `None`: field omitted (caller didn't request `--checksum`).
/// - `Some("")`: emitted as `null`.
/// - `Some(non-empty)`: emitted as the string verbatim.
///
/// Plain `Option<String>` can't represent the three states by
/// itself; this hook bridges the gap. Matches partisan's
/// `obj.checksum() is None` expectation
/// (`partisan/src/partisan/irods.py:2503`) — `item[Baton.CHECKSUM]`
/// returns the JSON value verbatim, so `null` gives Python `None`
/// while empty-string would give `""`.
fn serialize_checksum<S: Serializer>(
    ...
    match value {
        ...
        // Unreachable because `skip_serializing_if = "Option::is_none"`
        // filters `None` before this hook runs. Defensive arm in case
        // a future refactor drops the skip predicate without updating
        // this serializer.
...
pub struct BatonDoEnvelope {
    ...
    /// Top-level envelope fields baton-rs doesn't model.
    /// Captured on deserialise via the custom `visit_map` (see
    /// `impl Deserialize for BatonDoEnvelope`) and copied onto
    /// [`BatonDoOutput`] by [`BatonDoOutput::new`], so the
    /// response carries them verbatim.
    ///
    /// Why: upstream baton modifies the parsed input JSON in
    /// place (attaches `result` / `error`) and prints it back —
    /// see `baton/src/operations.c:130-150`, comments
    /// *"add ... to the input JSON as a property and print the
    /// input JSON"*. Downstream consumers exploit this: extendo
    /// (Go) round-trips a top-level `"ID"` field for
    /// request-response correlation, dropping responses that
    /// don't carry the right ID (`extendo/client.go:382-396`).
    /// Strip-and-rebuild on baton-rs's side would deviate from
    /// the upstream contract and silently break any such
    /// consumer.
    ///
    /// `BTreeMap` (rather than `HashMap`) gives deterministic
    /// key order on serialise, which keeps round-trip equality
    /// stable for tests and human eyeballs.
    #[serde(flatten)]
        ...
        D: Deserializer<'de>,
    {
        ...
        impl<'de> Visitor<'de> for EnvelopeVisitor {
                ...
                M: MapAccess<'de>,
            {
                ...
                // Unknown top-level keys land here so we can echo
                // them back on the output envelope. See the
                // `BatonDoEnvelope.extra` doc-comment for the
                // upstream-baton contract this satisfies.
                ...
                while let Some(key) = map.next_key::<String>()? {
                    match key.as_str() {
                        ...
                        _ => {
                            // Preserve fields baton-rs doesn't
                            // model so they round-trip onto the
                            // output envelope (upstream's
                            // "modify input JSON in place" pattern;
                            // see `BatonDoEnvelope.extra`).
...
pub struct BatonDoOutput {
    ...
    /// Top-level fields the original input envelope carried that
    /// baton-rs doesn't model. Echoed verbatim onto the response
    /// so downstream consumers that depend on round-trip
    /// fidelity (notably extendo's `"ID"` correlation field) get
    /// the same shape they'd see from upstream baton. Populated
    /// from [`BatonDoEnvelope.extra`] in [`BatonDoOutput::new`].
    /// See the matching `BatonDoEnvelope.extra` doc-comment for
    /// the upstream `baton/src/operations.c:130-150` citation.
    #[serde(flatten)]
    pub extra: BTreeMap<String, serde_json::Value>,
}
        ...
        D: Deserializer<'de>,
    {
        ...
        impl<'de> Visitor<'de> for OutputVisitor {
                ...
                M: MapAccess<'de>,
            {
                ...
                // Unknown top-level keys land here so tests that
                // parse an output line back into BatonDoOutput
                // observe the same round-tripped shape baton-rs
                // emits. See the matching
                // `BatonDoEnvelope`/`BatonDoOutput.extra`
                // doc-comments for the upstream contract.
                let mut extra: BTreeMap<String, serde_json::Value> = BTreeMap::new();
...
mod tests {
    ...
    #[test]
    fn data_object_checksum_none_omits_field_on_serialize() {
        // `Option<String>::None` skips serialisation (the
        // `skip_serializing_if` predicate fires before our custom
        // `serialize_checksum` hook). Wire shape: no `checksum` key.
        let mut d = DataObject {
        ...
        assert!(
            ...
            "checksum=None should be omitted from output, got {}",
            json
        );
    }

    #[test]
    fn data_object_checksum_some_empty_serializes_as_json_null() {
        // `Some("")` (`--checksum` requested but catalog has no
        // recorded value) emits `null` so partisan's
        // `obj.checksum() is None` assertion holds. Without this
        // hook, the empty string would serialize verbatim and
        // break partisan's None-check.
        let d = DataObject {
            collection: "/zone/home/u".to_string(),
            data_object: "foo.txt".to_string(),
            size: None,
        ...
        let json = serde_json::to_string(&d).unwrap();
        assert!(
            json.contains(r#""checksum":null"#),
            "checksum=Some(\"\") should serialize as JSON null, got {}",
            json
        );
    }

    #[test]
    fn data_object_checksum_some_value_serializes_as_string() {
        // `Some(non-empty)` emits the string verbatim — sanity that
        // our hook doesn't drop normal values.
        let d = DataObject {
            collection: "/zone/home/u".to_string(),
            data_object: "foo.txt".to_string(),
            size: None,
        ...
        let json = serde_json::to_string(&d).unwrap();
        assert!(
            json.contains(r#""checksum":"d41d8cd98f00b204e9800998ecf8427e""#),
            "Some(non-empty) should serialize verbatim, got {}",
            json
        );
    }
    ...
    #[test]
    fn data_object_with_local_file_distinct_from_data_object_round_trip() {
        // Per-record `file` distinct from `data_object` lets callers
        // stage bytes to a local name that differs from the iRODS-side
        // identifier — both extendo (`json:"file,omitempty"` on
        // `RodsItem`) and partisan (`item[Baton.FILE] = local_path.name`)
        // emit this shape. Round-trip pins the wire-format compat.
        let json = r#"{"collection":"/testZone/home/irods","data_object":"foo.txt","directory":"/tmp/baton_rs_save","file":"renamed_local.bin"}"#;
        let d: DataObject = serde_json::from_str(json).unwrap();
        assert_eq!(d.directory.as_deref(), Some("/tmp/baton_rs_save"));
        assert_eq!(d.file.as_deref(), Some("renamed_local.bin"));
        assert_eq!(d.data_object, "foo.txt");
        assert_eq!(serde_json::to_string(&d).unwrap(), json);
    }
    ...
    fn replicate_round_trip() {
        let json = r#"{"checksum":"d41d8cd98f00b204e9800998ecf8427e","location":"host","resource":"demoResc","number":0,"valid":true,"physical_path":"/var/lib/irods/Vault/home/irods/foo.txt"}"#;
        ...
        assert!(r.valid);
        assert_eq!(r.physical_path, "/var/lib/irods/Vault/home/irods/foo.txt");
    ...
    #[test]
    fn avu_query_accepts_array_value_for_in_operator() {
        // Partisan's `BatonJSONEncoder` (irods.py:3908-3915) passes
        // through `AVU.value` as-is; when an AVU is constructed
        // with `value=[...]` (used with operator="in") the wire
        // payload carries a JSON array. AvuQuery deserialises this
        // via the untagged AvuValue enum. See #57 follow-up /
        // partisan `test_data_object_examples`.
        let json = r#"{"attribute":"experiment_id","value":["exp_1","exp_2"],"operator":"in"}"#;
        let q: AvuQuery = serde_json::from_str(json).unwrap();
        assert_eq!(q.operator, Operator::In);
        match &q.value {
            AvuValue::Multi(v) => {
                assert_eq!(v, &vec!["exp_1".to_string(), "exp_2".to_string()])
            }
            other => panic!("expected Multi, got {:?}", other),
        }
        // Round-trip preserves the array form.
        assert_eq!(serde_json::to_string(&q).unwrap(), json);
    }

    #[test]
    fn avu_value_from_str_and_string_yield_single() {
        let a: AvuValue = "foo".into();
        let b: AvuValue = "foo".to_string().into();
        assert_eq!(a, AvuValue::Single("foo".to_string()));
        assert_eq!(b, AvuValue::Single("foo".to_string()));
    }

    #[test]
    fn avu_value_from_vec_yields_multi() {
        let v: AvuValue = vec!["a".to_string(), "b".to_string()].into();
        assert_eq!(v, AvuValue::Multi(vec!["a".to_string(), "b".to_string()]));
    }
    ...
    // --- Unknown-field round-trip (upstream `print_json(item)`)
    // -------------------------------------------------------------

    #[test]
    fn baton_do_envelope_captures_unknown_top_level_fields() {
        // Extendo's "ID" key (Go's default JSON name for the
        // `envelopeID.ID` embed at `extendo/client.go:97-100`) is
        // the load-bearing case: extendo's response reader at
        // `extendo/client.go:382-396` correlates responses by ID
        // and drops anything that doesn't echo it back. Same
        // contract applies to any future field a downstream
        // consumer adds; see the field doc-comment on
        // `BatonDoEnvelope.extra`.
        let json = r#"{
            "ID": 42,
            "operation": "list",
        ...
        let env: BatonDoEnvelope = serde_json::from_str(json).expect("parse");
        assert_eq!(env.extra.len(), 2);
        assert_eq!(env.extra.get("ID"), Some(&serde_json::json!(42)));
        assert_eq!(env.extra.get("trace_id"), Some(&serde_json::json!("abc-123")));
    }

    #[test]
    fn baton_do_envelope_round_trips_unknown_fields_at_top_level() {
        // Captured `extra` must serialise at the top level (not
        // nested under "extra") — that's what `#[serde(flatten)]`
        // is for. We round-trip the JSON via Value comparison so
        // key-order differences between serialisations don't make
        // the assertion flaky.
        ...
        let env: BatonDoEnvelope = serde_json::from_value(input.clone()).expect("parse");
        let round_tripped: serde_json::Value =
            serde_json::to_value(&env).expect("serialise");
        // Input has no `arguments`; the serialised form may or
        // may not, depending on whether Arguments::default() is
        // empty (it is, so the field skips). Compare the union:
        // round-tripped must be a superset of input that adds at
        // most the empty arguments block.
        for (k, v) in input.as_object().unwrap() {
            assert_eq!(
                round_tripped.get(k),
                Some(v),
                "key {} missing or wrong after round-trip; got {}",
                k,
                round_tripped
            );
        }
    ...
    #[test]
    fn baton_do_output_new_copies_extra_from_envelope() {
        // The crucial bit for the extendo deadlock: ID lands on
        // the envelope's `extra`, then `BatonDoOutput::new` copies
        // it onto the output so the wire response carries the same
        // top-level "ID" extendo's reader correlates on.
        let env: BatonDoEnvelope = serde_json::from_str(
            r#"{"ID": 7, "operation": "list", "target": {"collection": "/x"}}"#,
        )
        .unwrap();
        let out = BatonDoOutput::new(env, None, None);
        assert_eq!(out.extra.get("ID"), Some(&serde_json::json!(7)));

        // And the wire form has "ID" at the top alongside operation
        // (not nested under "extra"), so extendo's `var envID
        // envelopeID; json.Unmarshal(line, &envID)` picks it up.
        let wire = serde_json::to_value(&out).unwrap();
        assert_eq!(wire.get("ID"), Some(&serde_json::json!(7)));
    }

    #[test]
    fn baton_do_envelope_with_no_extras_keeps_extra_empty() {
        // Partisan-style envelope: no ID, no extras. Extra map
        // stays empty; output omits "ID" entirely. Same skip
        // behaviour an empty Arguments block gets.
        let env: BatonDoEnvelope = serde_json::from_str(
            r#"{"operation": "list", "target": {"collection": "/x"}}"#,
        )
        .unwrap();
        ...
        assert!(
            wire.get("ID").is_none(),
            "no ID on input → no ID on output: {}",
            wire
        );
    }
