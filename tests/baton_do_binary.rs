...
#[test]
fn parse_failure_partisan_shape_emits_error_without_deadlocking() {
    // Regression test for #37. Partisan sends one envelope per
    // request with no trailing newline, then blocks on
    // stdout.readline() waiting for baton-do's response.
    // Pre-fix, the streaming reader's recovery path called
    // `advance_past_newline` which did a blocking
    // `read_until(b'\n', ...)` — with no newline in the input
    // and no more bytes coming until partisan sees a response,
    // both sides deadlocked.
    //
    // We send a well-formed JSON envelope with an invalid
    // `operation` discriminator (so it deserialises through the
    // enum-visitor failure path, not the JSON-syntax path) and
    // no trailing newline. The fix emits the synthetic
    // `{"error": ...}` line promptly and exits at EOF.
    //
    // If the deadlock regresses, this test will hang until
    // CI's per-job timeout kills it — that's a louder failure
    // signal than a clean assert, which is what we want for
    // this class of bug.
    let input = r#"{"operation":"frobnicate","target":{"collection":"/x"}}"#;
    assert!(
        !input.contains('\n'),
        "test input must contain no newlines to exercise the no-newline recovery path"
    );

    let output = run_baton_do(&[], input);
    assert!(
        !output.status.success(),
        "unknown operation should produce non-zero exit",
    );
    ...
    assert_eq!(
        lines.len(),
        1,
        "exactly one synthetic error line, no hang: {:?}",
        lines
    );
    assert!(
        lines[0].get("error").is_some(),
...
#[test]
fn envelope_echoes_unknown_top_level_id_field() {
    // Extendo (Go) embeds an `envelopeID.ID uint64` in every
    // envelope it sends and correlates responses by reading just
    // that field back out of each stdout line
    // (`extendo/client.go:382-396`). Upstream baton round-trips
    // unknown envelope fields by modifying the parsed input JSON
    // in place and printing it back (`baton/src/operations.c:130-150`,
    // `print_json(item)`). baton-rs's previous strip-and-rebuild
    // approach dropped the ID, the response came back with
    // `envID.ID = 0`, no extendo waiter matched, and the
    // originating goroutine blocked forever — Ginkgo's
    // BeforeEach hit a 5-minute timeout on the first MkDir spec.
    //
    // We use a raw JSON line rather than `BatonDoEnvelope`'s
    // constructor because the constructor doesn't populate
    // `extra` — we want to exercise the wire-shape path a real
    // Go-side client takes.
    let name = unique_name("baton_rs_bin_id_echo");
    let local = "/tmp/baton_rs_bin_id_echo_src";
    std::fs::write(local, b"binary id echo").expect("write");
    let remote = format!("{}/{}", TEST_COLL, name);
    iput(local, &remote);
    let _cleanup = IrodsCleanup(remote);

    let input = format!(
        r#"{{"ID": 42, "operation": "list", "target": {{"collection": "{}", "data_object": "{}"}}}}"#,
        TEST_COLL, name,
    );
    let output = run_baton_do(&[], &input);
    assert_success_exit(&output);
    ...
    let lines = parse_value_lines(&stdout_str(&output));
    assert_eq!(lines.len(), 1, "expected one response line: {:?}", lines);
    assert_eq!(
        lines[0].get("ID"),
        Some(&Value::from(42)),
        "response must echo the input ID field; got {}",
        lines[0],
    );
    assert!(
        lines[0].get("operation").is_some(),
        "response should still carry operation: {}",
        lines[0],
    );
    assert!(
        lines[0].get("result").is_some(),
        "response should carry result: {}",
        lines[0],
    );
}
