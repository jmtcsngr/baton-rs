...
use baton_rs::types::{
    AccessQuery, AclLevel, AvuQuery, AvuValue, MetaqueryInput, Operator, TimestampQuery,
};
...
fn metaquery_timestamp_range_returns_recent_object() {
    ...
    let input_match = MetaqueryInput {
        ...
        timestamps: vec![TimestampQuery {
            // Epoch zero in ISO 8601. The metaquery layer converts
            // to seconds before building the SQL — see #57 Cluster C
            // and the module-level docs in operations/metaquery.rs.
    ...
    let input_miss = MetaqueryInput {
        ...
        timestamps: vec![TimestampQuery {
            // 9999999999 epoch seconds = 2286-11-20T17:46:39Z. Same
            // "no put can satisfy this for centuries" intent as
            // before, just expressed in the ISO 8601 form the
            // metaquery layer now expects.
            created: Some("2286-11-20T17:46:39Z".to_string()),
            modified: None,
            operator: Operator::GreaterThanOrEqual,
        }],
        access: vec![],
...
#[test]
fn metaquery_in_operator_with_array_value_matches_any() {
    // Partisan's `test_examples.py::test_data_object_examples`
    // hangs pre-fix on this exact wire shape: an IN operator with
    // a JSON array of candidate values. Pre-fix, `AvuQuery.value`
    // was a `String` and partisan's array failed to deserialise,
    // so baton-do silently dropped the envelope (per #37) and
    // partisan's `stdout.readline()` blocked indefinitely. Fixed
    // by accepting `AvuValue::Multi(Vec<String>)` and rendering it
    // as iRODS' `('a', 'b', 'c')` literal in `condition_for`.
    let local = "/tmp/baton_rs_metaquery_in";
    std::fs::write(local, b"metaquery in").expect("write");

    let remote_coll = "/testZone/home/irods";
    let data_object = "baton_rs_metaquery_in";
    let remote = format!("{}/{}", remote_coll, data_object);
    iput(local, &remote);
    let _cleanup = IrodsCleanup(remote.clone());

    imeta_add("-d", &remote, "baton_rs_metaquery_in_attr", "exp_1", None);
    ...
    let input = MetaqueryInput {
        avus: vec![AvuQuery {
            attribute: "baton_rs_metaquery_in_attr".to_string(),
            // The IN-array shape partisan emits for
            // `AVU(name, [v1, v2], operator="in")`. Only one of the
            // values needs to be present on the object for it to
            // match.
            value: AvuValue::Multi(vec!["exp_1".to_string(), "exp_2".to_string()]),
            units: None,
    ...
    .expect("metaquery with IN-array value");
    ...
    assert!(
        found,
        "expected data object {} in IN-array results, got {:?}",
        remote, results
    );
}
