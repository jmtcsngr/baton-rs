...
fn main() {
    ...
    shim.file("shim/ffi_shim.c")
        .include("shim")
        .include("/usr/include/irods")
        .warnings(true);

    // Compile the test-only fault-injection seam (issue #108) only when
    // built with `--features fault-injection`. Cargo exports
    // CARGO_FEATURE_FAULT_INJECTION for that feature; mirror it into a C
    // define. Release/publish builds pass no features, so the seam is
    // never compiled in.
    println!("cargo:rerun-if-env-changed=CARGO_FEATURE_FAULT_INJECTION");
    if std::env::var_os("CARGO_FEATURE_FAULT_INJECTION").is_some() {
        shim.define("BATON_RS_FAULT_INJECTION", None);
    }

    shim.compile("baton_rs_shim");
